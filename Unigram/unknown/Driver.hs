...
replaceWithUnk :: UnigramMap -> UnigramMap
replaceWithUnk m@(UM m') = addMany lonelyPOSList $ deleteMany lonelyPOSList m
  where
    lonelyPOSList :: [TaggedPOS]
    lonelyPOSList = fmap (\(a,b) -> (fst . head . Map.toList $ b, a)) . filter ((< 2) . countTags . snd) . Map.toList $ m'

    deleteMany :: [TaggedPOS] -> UnigramMap -> UnigramMap
    deleteMany tps (UM m) = UM $ foldl (\acc (_, p) -> Map.delete p acc) m tps

    addMany :: [TaggedPOS] -> UnigramMap -> UnigramMap
    addMany tps m = foldl (\acc (t, _) -> add (t, "UNK") acc) m tps

    countTags :: Map.Map Tag Int -> Int
    countTags = foldr (+) 0
...
main =
  ...
  pure . replaceWithUnk >>=       -- Mark uncommon words
